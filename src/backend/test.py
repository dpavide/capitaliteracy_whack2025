...
# Set the email (you can also read from a JSON or input() for testing)
email = "example@email.com"  # Replace or set dynamically

# Get user info from backend
def get_user_info(email: str):
    try:
        r = requests.post(f"{API_URL}/api/user-info", json={"email": email}, timeout=10)
        r.raise_for_status()
        data = r.json()
        return (data["age"], data["numerical_literacy_level"], data["credit_score"])
    except Exception as e:
        print("Error fetching user info:", e)
        return None

user_info = get_user_info(email)
print("Fetched User Info Tuple:", user_info)
...
goal = "I want to buy a car that costs 10,000"
...
The current numerical literacy is {numerical_level}
