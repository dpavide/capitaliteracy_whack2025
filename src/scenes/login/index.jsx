// src/pages/auth/Login.jsx  (or wherever you keep it)
...
// IMPORTANT: set this to the exact column name(s) in your DB (must match signup)
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!user) {
        setAuthError('No user returned from sign-in. Please try again.');
        setLoading(false);
        return;
      }

      // store userId in localStorage for convenience (NOT a secret)
      try {
        localStorage.setItem('userId', user.id);
      } catch (err) {
        // ignore storage errors - still continue
        console.warn('Could not persist userId to localStorage:', err);
      }

      // Fetch profile fields in one go (age, credit_score, numerical_literacy_level)
      let profile = null;
      const selectCols = `id, age, credit_score, ${NUMERICAL_LITERACY_COLUMN}`;
      ...
      if (profileErr) {
        ...
        console.warn('Profile read error:', profileErr.message ?? profileErr);
      }
      profile = p ?? null;
      ...
      if (!profile && pendingRaw) {
        try {
          ...
          console.warn('pendingProfile parse/insert failed, continuing without it:', err);
        }
      } else if (pendingRaw && profile) {
        // clean up stale pendingProfile if user now has profile
        try {
          localStorage.removeItem('pendingProfile');
        } catch {}
      }

      // enforce minimum age if present
      ...
      // If we have profile data, extract tuple and store in sessionStorage for current session
      if (profile) {
        const creditScore = profile.credit_score ?? null;
        const numericalLiteracy = profile[NUMERICAL_LITERACY_COLUMN] ?? null;
        const tuple = [profile.age ?? null, creditScore, numericalLiteracy];

        try {
          // sessionStorage is preferred for profile details (cleared when tab/window closed)
          sessionStorage.setItem('profile_tuple', JSON.stringify(tuple));
        } catch (err) {
          // fallback to localStorage if sessionStorage unavailable
          try {
            localStorage.setItem('profile_tuple', JSON.stringify(tuple));
          } catch (err2) {
            console.warn('Could not persist profile tuple:', err2);
          }
        }
      } else {
        // If profile not present, ensure any previous stored tuple is removed
        try {
          sessionStorage.removeItem('profile_tuple');
        } catch {}
      }
